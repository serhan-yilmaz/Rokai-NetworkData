...
%%
ptmcode_version = '2014-09-17';
species_list = {'human', 'mouse', 'rat'};

for iSpecies = 1:length(species_list)
    species = species_list{iSpecies};
    fprintf('[Running] Parsing PTMCode Networks - %s\n', species)
    ...
    switch(species)
        ...
        case 'rat'
            species_code = 'Rattus norvegicus';
        otherwise
            error('Unknown species code.');
    end
    
    % Filter for mouse
    validRows = strcmpi(T1.Species, species_code);
    T1 = T1(validRows, :);
    ...
    % Filter for mouse
    validRows = strcmpi(T2.Species, species_code);
    T2 = T2(validRows, :);
    ...
    outputFolder =  '../data/processed/ptmcode/';
    if(~exist(outputFolder, 'dir')); mkdir(outputFolder); end
    save([outputFolder, 'ptmcode_networks_', species, '.mat'], ...
        'Wcoev', 'Wsd', 'Site', 'Gene', 'ptmcode_version');
end
fprintf('[Done] Parsing PTMCode Networks\n');
